      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardContent className="p-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-slate-300">Current</span>
              <span className="text-white font-medium">{teamStats.rosterSize} / 17 players</span>
            </div>
            <Progress value={(teamStats.rosterSize / 17) * 100} className="h-2 bg-slate-700" />

            {hasPendingBids && (
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">Projected</span>
                  <span className={`font-medium ${projectedRosterSize > 17 ? "text-red-400" : "text-slate-400"}`}>
                    {projectedRosterSize} / 17 players
                  </span>
                </div>
                <Progress value={(projectedRosterSize / 17) * 100} className="h-2 bg-slate-700 opacity-60" />
              </>
            )}
          </div>
        </CardContent>
