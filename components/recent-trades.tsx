  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch players without position data from users table
        ...
        // Get current season for position data
        const currentSeasonId = getCurrentSeasonId()
        let seasonUUID = null
        let seasonNumberForPositions = null

        if (typeof currentSeasonId === "string") {
          // It's already a UUID
          seasonUUID = currentSeasonId
        } else {
          // It's a season number, get the UUID
          const { data: seasonData } = await supabase
            .from("seasons")
            .select("id, season_number, name, parent_season_id")
            .eq("season_number", currentSeasonId)
            .single()

          if (seasonData) {
            seasonUUID = seasonData.id
            // Check if it's a playoff season and use parent season for positions
            if (seasonData.name?.includes("(Playoffs)") && seasonData.parent_season_id) {
              const { data: parentSeasonData } = await supabase
                .from("seasons")
                .select("season_number")
                .eq("id", seasonData.parent_season_id)
                .single()

              seasonNumberForPositions = parentSeasonData?.season_number || currentSeasonId
            } else {
              seasonNumberForPositions = seasonData.season_number
            }
          }
        }

        // Fetch season registrations for position data
        let seasonRegistrations: any[] = []
        if (seasonNumberForPositions) {
          const { data: seasonRegsData } = await supabase
            .from("season_registrations")
            .select("user_id, primary_position, secondary_position")
            .eq("season_number", seasonNumberForPositions)

          seasonRegistrations = seasonRegsData || []
        }

        // Create season registration map
        const seasonRegMap: Record<string, any> = {}
        seasonRegistrations.forEach((reg) => {
          seasonRegMap[reg.user_id] = reg
        })

        const playerMap: Record<string, any> = {}
        playersData?.forEach((player) => {
          const seasonReg = seasonRegMap[player.user_id]
          const playerWithPositions = {
            ...player,
            users: {
              ...player.users,
              primary_position: seasonReg?.primary_position || "UNKNOWN",
              secondary_position: seasonReg?.secondary_position || "",
            },
          }
          playerMap[player.id] = playerWithPositions
        })
        ...
        (payload) => {
          if (payload.new && (payload.new as any).status === "accepted") {
            setTrades((currentTrades) => {
              const updatedTrades = [payload.new, ...currentTrades.filter((t) => t.id !== (payload.new as any).id)]
              updatedTrades.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
              return updatedTrades.slice(0, 5)
            })
          } else if (payload.old && (payload.new as any)?.status !== "accepted") {
            // Remove trade if status changed from accepted
            setTrades((currentTrades) => currentTrades.filter((t) => t.id !== (payload.old as any).id))
          }
        },
      )
      .subscribe()
  ...
  if (trades.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground text-center py-4">No accepted trades found.</p>
        </CardContent>
      </Card>
    )
  }
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        <div className="space-y-1">
          <CardTitle>Recent Trades</CardTitle>
          <CardDescription>Latest accepted trades around the league</CardDescription>
        </div>
