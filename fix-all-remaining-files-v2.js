...
function fixFile(filePath) {
  try {
    ...
    // Skip if already fixed (contains our fix pattern)
    if (content.includes('supabaseUrl && supabaseKey')) {
      return;
    }
    ...
    // Pattern 7: createClient(process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || "", process.env.SUPABASE_SERVICE_ROLE_KEY || "", {
    const pattern7 = /const\s+(\w+)\s*=\s*createClient\(\s*process\.env\.SUPABASE_URL\s*\|\|\s*process\.env\.NEXT_PUBLIC_SUPABASE_URL\s*\|\|\s*""\s*,\s*process\.env\.SUPABASE_SERVICE_ROLE_KEY\s*\|\|\s*""\s*,\s*\{([^}]*)\}\s*\)/g;
    
    // Pattern 8: createClient(process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || "", process.env.SUPABASE_SERVICE_ROLE_KEY || "")
    const pattern8 = /const\s+(\w+)\s*=\s*createClient\(\s*process\.env\.SUPABASE_URL\s*\|\|\s*process\.env\.NEXT_PUBLIC_SUPABASE_URL\s*\|\|\s*""\s*,\s*process\.env\.SUPABASE_SERVICE_ROLE_KEY\s*\|\|\s*""\s*\)/g;
...
const ${varName} = supabaseUrl && supabaseServiceKey 
  ...
  : null`;
    ...
    // Fix pattern 7
    content = content.replace(pattern7, (match, varName, config) => {
      modified = true;
      return `// Check if environment variables are available
...
const supabaseFiles = allFiles.filter(file => {
  try {
    ...
    return content.includes('createClient') && 
           ...
           !content.includes('supabaseUrl && supabaseKey'); // Skip already fixed files
  } catch (error) {
    return false;
  }
});

console.log(`Found ${supabaseFiles.length} files with Supabase client creation that need fixing`);
