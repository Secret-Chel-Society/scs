...
import { Loader2, Check, AlertCircle, Upload, RefreshCw, Info, Palette } from "lucide-react"
      ...
      <div className="container mx-auto">
        <div className="mb-8">
          ...
          <p className="text-white/70 text-lg">
            Manage team logos and storage for all teams
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-white/70">Update all team logos from storage at once</p>
              </div>
              <Button 
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-4 bg-slate-800/50 border border-white/20">
            ...
            <TabsTrigger value="debug" className="text-white data-[state=active]:bg-purple-500/20 data-[state=active]:text-purple-400">Debug Info</TabsTrigger>
            {updateResults.length > 0 && <TabsTrigger value="results" className="text-white data-[state=active]:bg-orange-500/20 data-[state=active]:text-orange-400">Update Results</TabsTrigger>}
          </TabsList>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {teams.map((team) => {
                        ...
                        return (
                          <TableRow key={team.id} className="border-white/20 hover:bg-slate-800/30">
                                ...
                                {team.logo_url ? (
                                  <>
                                    ...
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      className="absolute -top-2 -right-2 h-6 w-6 p-0 rounded-full text-white hover:bg-slate-700/50"
                            ...
                            <TableCell>
                              {hasStorageLogo ? (
                                ...
                                <span className="text-white/70">No storage logo</span>
                              )}
                            </TableCell>
                            <TableCell>
                              {!hasStorageLogo ? (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {Object.entries(TEAM_LOGOS).map(([teamName, filename]) => {
                        ...
                        return (
                          <TableRow key={teamName} className="border-white/20 hover:bg-slate-800/30">
                            <TableCell className="font-medium text-white">{teamName}</TableCell>
                            <TableCell className="text-white/80">teams/{filename}</TableCell>
                            <TableCell>
                              <div className="h-16 w-16 relative">
                                {storageUrl ? (
                                  <>
                                  ...
                                  <div className="flex items-center justify-center h-full w-full bg-slate-800/50 rounded border border-white/20">
                    ...
                    <TableBody>
                      {Object.entries(debugInfo).map(([teamName, info]) => (
                        <TableRow key={teamName} className="border-white/20 hover:bg-slate-800/30">
                          <TableCell className="font-medium text-white">{teamName}</TableCell>
                          <TableCell>
                            <code className="text-xs text-white/80">{info.storagePath}</code>
                          </TableCell>
                          <TableCell>
                            <div className="max-w-[200px] truncate">
                              <code className="text-xs text-white/80">{info.publicUrl || "N/A"}</code>
                            </div>
                          </TableCell>
                          <TableCell>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {updateResults.map((result, index) => (
                          <TableRow key={index} className="border-white/20 hover:bg-slate-800/30">
        ...
        <Dialog open={!!selectedImage} onOpenChange={(open) => !open && setSelectedImage(null)}>
          <DialogContent className="max-w-3xl bg-slate-800 border border-white/20">
            <DialogHeader>
            ...
            <div className="flex justify-center p-4 bg-slate-700/50 rounded-md border border-white/20">
              {selectedImage && (
