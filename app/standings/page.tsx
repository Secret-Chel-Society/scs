    ...
    <div className="grid gap-6 lg:grid-cols-2">
      {Object.entries(divisions).map(([divisionName, teams]) => (
        <Card key={divisionName}>
          ...
          <CardContent>
            <TeamStandings teams={sortTeams(teams)} />
          </CardContent>
        </Card>
      ))}
    </div>
...
function SeasonStandings({
  ...
  season: { id: number; name: string; is_active: boolean }
...
export default function StandingsPage() {
  const [seasons, setSeasons] = useState<{ id: number; name: string; is_active: boolean }[]>([])
  const [standingsData, setStandingsData] = useState<{ [seasonId: number]: TeamStanding[] }>({})
      ...
      try {
        ...
        if (seasonsData && seasonsData.length > 0) {
          ...
          const standingsMap = allStandings.reduce(
            ...
            {} as { [seasonId: number]: TeamStanding[] },
          )
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Team standings, conference rankings, division rankings, and playoff picture for all seasons
            </p>
          </div>
          ...
          <Card>
            <CardContent className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mt-2">No seasons found in the database.</p>
              </div>
            </CardContent>
          </Card>
        </div>
  ...
  const filteredSeasons =
    selectedSeason === "all" ? sortedSeasons : sortedSeasons.filter((season) => season.id.toString() === selectedSeason)

  return (
    <div className="container mx-auto px-3 md:px-4 py-4 md:py-8">
      <div className="space-y-4 md:space-y-6">
        <div className="space-y-1 md:space-y-2">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">League Standings</h1>
        ...
        <div className="space-y-6 md:space-y-8">
          {filteredSeasons.map((season) => (
            <SeasonStandings key={season.id} season={season} standings={standingsData[season.id] || []} />
          ))}
        </div>
      </div>
    </div>
