...
export async function POST(request: Request) {
  try {
    ...
    console.log("🚀 Running IP tracking migration...")
    ...
    try {
      ...
      if (columnError) {
        ...
        results.push({ step: "Add columns", success: false, error: columnError.message })
      } else {
        console.log("✅ Successfully added IP columns to users table")
        results.push({ step: "Add columns", success: true })
      }
    } catch (error: any) {
      console.error("❌ Exception adding columns:", error)
      results.push({ step: "Add columns", success: false, error: error.message })
    }
    ...
    try {
      ...
      if (tableError) {
        ...
        results.push({ step: "Create table", success: false, error: tableError.message })
      } else {
        console.log("✅ Successfully created ip_logs table")
        results.push({ step: "Create table", success: true })
      }
    } catch (error: any) {
      console.error("❌ Exception creating table:", error)
      results.push({ step: "Create table", success: false, error: error.message })
    }
    ...
    try {
      ...
      if (indexError) {
        ...
        results.push({ step: "Create indexes", success: false, error: indexError.message })
      } else {
      ...
      results.push({ step: "Create indexes", success: false, error: error.message })
    }
    ...
    try {
      ...
      if (functionError) {
        ...
        results.push({ step: "Create function", success: false, error: functionError.message })
      } else {
        console.log("✅ Successfully created log_ip_address function")
        results.push({ step: "Create function", success: true })
      }
    } catch (error: any) {
      console.error("❌ Exception creating function:", error)
      results.push({ step: "Create function", success: false, error: error.message })
    }
    ...
    try {
      ...
      const { data: testResult, error: testError } = await supabase.rpc('log_ip_address', {
        ...
        p_action: 'test',
        p_user_agent: 'migration-test'
      })

      if (testError && !testError.message.includes('violates foreign key constraint')) {
        console.error("❌ Function test failed:", testError)
        results.push({ step: "Test function", success: false, error: testError.message })
      } else {
        console.log("✅ Function test successful")
        results.push({ step: "Test function", success: true })
      }
    } catch (error: any) {
      console.error("❌ Exception testing function:", error)
      results.push({ step: "Test function", success: false, error: error.message })
    }
    ...
    return NextResponse.json({
      ...
      results,
      nextSteps: successCount === totalSteps ? [
        "IP tracking migration completed successfully",
        "New registrations and logins will now track IP addresses",
        "Existing users will show 'Unknown' until they log in again"
      ] : [
        "Some migration steps failed",
        "Check the error messages above",
        "You may need to run the migration again or fix database permissions"
      ]
    })

  } catch (error: any) {
    console.error("❌ Error running IP migration:", error)
    return NextResponse.json({
      success: false,
      error: error.message
    }, { status: 500 })
