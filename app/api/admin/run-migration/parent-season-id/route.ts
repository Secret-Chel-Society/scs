...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("🚀 Starting parent_season_id migration...")
...
COMMENT ON FUNCTION create_playoff_season(UUID, TEXT) IS 'Creates a playoff season linked to a parent regular season';
    ...
    console.log("✅ Parent season ID migration completed successfully")

    // Update existing playoff seasons to have proper parent relationships
    ...
    if (seasonsError) {
      console.error("❌ Error fetching existing seasons:", seasonsError)
    } else {
      console.log(`📊 Found ${existingSeasons?.length || 0} existing seasons`)
      
      // Update any existing playoff seasons to have proper parent relationships
      for (const season of existingSeasons || []) {
        if (season.name?.toLowerCase().includes("playoff")) {
          // Find the corresponding regular season
          const regularSeason = existingSeasons?.find(s => 
            s.number === season.number && 
            !s.name?.toLowerCase().includes("playoff")
          )
          
          if (regularSeason) {
            console.log(`🔗 Linking playoff season "${season.name}" to parent "${regularSeason.name}"`)
            
            await supabase
              .from("seasons")
              .update({
                parent_season_id: regularSeason.id,
                is_playoff: true
              })
              .eq("id", season.id)
          }
    ...
    return NextResponse.json({
      success: true,
      message: "Parent season ID migration completed successfully",
      details: {
        addedColumns: ["parent_season_id", "is_playoff", "playoff_start_date", "playoff_end_date"],
        createdFunctions: ["link_playoff_registrations", "get_combined_season_stats", "create_playoff_season"],
        createdView: "combined_season_stats",
        createdTrigger: "trigger_link_playoff_registrations"
      }
    })
  } catch (error: any) {
