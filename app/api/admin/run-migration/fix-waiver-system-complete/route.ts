...
export async function POST() {
  try {
    ...
    try {
      const filePath = join(process.cwd(), "fix_waiver_system_complete.sql")
      sqlContent = await readFile(filePath, "utf-8")
    } catch (fileError) {
      // If file doesn't exist, use hardcoded SQL
      console.log("Using hardcoded SQL - file not found")
      sqlContent = `
...
$$;
    ...
    return NextResponse.json({
      success: true,
      message: "Waiver system migration completed successfully",
      priorityRecords: priorityData?.length || 0,
      claimsRecords: claimsData?.length || 0,
    })

  } catch (error: any) {
    console.error("Error running waiver system migration:", error)
    return NextResponse.json({ 
      error: "Migration failed", 
