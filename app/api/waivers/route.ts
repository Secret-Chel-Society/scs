...
export async function POST(request: NextRequest) {
  try {
    ...
    // Move the player off the active NHL roster into tc_team_id (holding state while on waivers).
    // team_id → tc_team_id, is_tc: true. NHL uses called_up_at (timestamptz), no boolean.
    const { error: moveToTcError } = await supabase
      .from("players")
      .update({
        team_id: null,
        tc_team_id: targetPlayer.team_id,
        is_tc: true,
        called_up_at: null,
        status: "waived",
      })
    ...
    if (moveToTcError) {
      console.error("Error moving player to TC on waiver:", moveToTcError)
      return NextResponse.json({ error: "Failed to update player status: " + moveToTcError.message }, { status: 500 })
    }
    ...
    try {
      ...
      await logActivity(supabase, {
        ...
        actorType,
        actionType: "waiver_placed",
        actionDescription: `${actorUser?.gamer_tag_id || "Manager"} placed ${playerUser?.gamer_tag_id || "player"} on waivers from ${team?.name || "team"}`,
        targetId: playerId,
      ...
      console.error("Error logging waiver activity:", logError)
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Note: position info is in season_registrations, not users table
    ...
    // Get user_ids from all players to fetch their season registrations
    const userIds = validWaivers
      .map((w) => w.players?.user_id)
    ...
    if (userIds.length > 0) {
      ...
      if (registrations) {
        // Create a map of user_id to their most recent registration
        registrations.forEach((reg) => {
          if (!registrationsMap[reg.user_id]) {
            registrationsMap[reg.user_id] = reg
          }
        })
      }
    }

    // Enrich waivers with season registration data
    const enrichedWaivers = validWaivers.map((waiver) => {
      const userId = waiver.players?.user_id
      const registration = userId ? registrationsMap[userId] : null

      return {
        ...waiver,
        players: {
          ...waiver.players,
          season_registrations: registration ? [registration] : [],
        },
      }
    })

    return NextResponse.json({ waivers: enrichedWaivers })
  } catch (error: any) {
