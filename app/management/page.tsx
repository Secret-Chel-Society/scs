...
// icons kept (some used in header/tab components)
import { Home, Gavel, Users, Calendar } from "lucide-react"
...
export default function ManagementPage() {
      ...
      try {
        ...
        if (processResponse.ok) {
          ...
          if (processResult.expiredCount > 0) {
            toast({ title: "Waivers Processed", description: `${processResult.expiredCount} expired waivers have been processed.` })
            await fetchData()
          }
        }
      } catch (e) {
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (myTeamBids) {
        const { data: allBids } = await supabase.from("player_bidding").select("*").order("bid_amount", { ascending: false })
