...
import { AhlEaMatchImportModal } from "@/components/matches/ahl-ea-match-import-modal"
...
export function MatchDetails({ matchId }: MatchDetailsProps) {
  ...
  try {
    ...
    // 2) Fetch teams explicitly
    const [{ data: homeTeam }, { data: awayTeam }] = await Promise.all([
      supabase.from("teams_ahl").select("id,name,logo_url,ea_club_id").eq("id", matchRow.home_team_id).maybeSingle(),
      supabase.from("teams_ahl").select("id,name,logo_url,ea_club_id").eq("id", matchRow.away_team_id).maybeSingle(),
    ])
    ...
    // Optional: attach season name (safe)
    if (matchRow.season_id) {
      const { data: seasonData } = await supabase
        ...
        .maybeSingle()
      if (seasonData?.name) setSeasonName(seasonData.name)
    }

    // Debug info stays the same
    setDebugInfo({
      matchData,
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-8 py-4">
            <div className="flex flex-col items-center">
              <div className="relative w-20 h-20 mb-2">
                <Link href={`/ahl/teams/${match.home_team?.id || "#"}`}>
                  <Image
            ...
            <div className="flex flex-col items-center">
              <div className="relative w-20 h-20 mb-2">
                <Link href={`/ahl/teams/${match.away_team?.id || "#"}`}>
                  <Image
