...
export default function AHLMatchesPage() {
  ...
  useEffect(() => {
    async function fetchTeams() {
      try {
        const { data, error } = await supabase.from("teams_ahl").select("id, name").eq("is_active", true).order("name")

        if (error) throw error
        setTeams(data || [])
      } catch (error) {
        console.error("Error fetching AHL teams:", error)
      }
    }
  ...
  useEffect(() => {
    async function fetchMatches() {
      ...
      try {
        ...
        if (selectedTeam !== "all") {
          matchQuery = matchQuery.or(`home_team_id.eq.${selectedTeam},away_team_id.eq.${selectedTeam}`)
        }

        const { data, error } = await matchQuery.order("match_date", { ascending: true })

        if (error) {
          // If foreign key relationship doesn't exist, fall back to separate queries
          if (error.message.includes("relationship")) {
            console.log("Foreign keys not set up yet, fetching teams separately")
            ...
            if (selectedTeam !== "all") {
              fallbackQuery = fallbackQuery.or(`home_team_id.eq.${selectedTeam},away_team_id.eq.${selectedTeam}`)
            }

            const { data: matchesData, error: matchesError } = await fallbackQuery.order("match_date", {
              ascending: true,
            })
            ...
            console.log(`Found ${matchesWithTeams.length} AHL matches for ${currentSeason.name}`)
            setMatches(matchesWithTeams)

            if (matchesWithTeams && matchesWithTeams.length > 0) {
              const weeks = calculateWeeks(matchesWithTeams)
              setTotalWeeks(weeks)

              if (currentWeek === null && !searchParams.get("week")) {
                const calculatedWeek = calculateCurrentWeek(matchesWithTeams)
        ...
        console.log(`Found ${data?.length || 0} AHL matches for ${currentSeason.name}`)
        setMatches(data || [])
        ...
        if (data && data.length > 0) {
          ...
          if (currentWeek === null && !searchParams.get("week")) {
            const calculatedWeek = calculateCurrentWeek(data)
            setCurrentWeek(calculatedWeek)
          } else if (currentWeek === null) {
            setCurrentWeek(1)
          }
        ...
        console.error("Error fetching AHL matches:", error)
        setError(`Error: ${error.message}`)
        toast({
          title: "Error",
          description: "Failed to load AHL matches. Please try again.",
          variant: "destructive",
        })
        setMatches([])
      } finally {
  ...
  const calculateCurrentWeek = (matchesData: any[]) => {
    if (!matchesData.length) return 1
    ...
    const daysSinceStart = Math.floor((today.getTime() - firstMatchDate.getTime()) / (24 * 60 * 60 * 1000))
  ...
  const updateURL = (week: number, team: string) => {
    ...
    const newURL = params.toString() ? `/ahl/matches?${params.toString()}` : "/ahl/matches"
      ...
      <div className="min-h-screen relative">
        ...
        <div className="relative z-10 container mx-auto px-4 py-8">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg">Follow all the action from your favorite AHL teams</p>
          </div>
      ...
      <div className="min-h-screen relative">
        ...
        <div className="relative z-10 container mx-auto px-4 py-8">
          ...
          <div className="max-w-md mx-auto bg-card border border-destructive/20 rounded-xl p-8 text-center">
            ...
            <h2 className="text-2xl font-bold mb-2">Unable to Load AHL Matches</h2>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center">
                <div className="flex items-center gap-2">
                  <Select value={selectedTeam} onValueChange={handleTeamFilter}>
                    ...
                    <SelectContent>
                      <SelectItem value="all">🏒 All AHL Teams</SelectItem>
                      {teams.map((team) => (
                ...
                {dateMatches.map((match) => {
                  ...
                  return (
                    <Card
                      ...
                      onClick={() => router.push(`/ahl/matches/${match.id}`)}
                    >
