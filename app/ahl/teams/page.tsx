...
export default function AHLTeamsPage() {
  ...
  useEffect(() => {
    async function fetchAHLTeams() {
      try {
        ...
        if (!currentSeason) {
          throw new Error("No AHL season found")
        }

        console.log("[v0] Using AHL season:", currentSeason.name, "ID:", currentSeason.id)

        const teamStats = await getAllAHLTeamStats(currentSeason.id)

        console.log("[v0] Got", teamStats.length, "teams from AHL standings calculator")
        ...
        const { data: awards, error: awardsError } = await supabase
          ...
          .in(
            "team_id",
            teamStats.map((t) => t.id),
          )

        if (awardsError) {
          console.warn("Could not fetch AHL awards:", awardsError)
        }
        ...
        console.log("[v0] Final teams with awards:", teamsWithAwards.length)

        setTeams(teamsWithAwards)
      } catch (error: any) {
        console.error("[v0] Error loading AHL teams:", error)
        toast({
          title: "Error loading AHL teams",
          description: error.message || "Failed to load AHL teams data.",
          variant: "destructive",
        })
      } finally {
        setLoading(false)
    ...
    <div className="container mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold mb-2">AHL Teams</h1>
            <p className="text-muted-foreground">All teams competing in the American Hockey League</p>
          </div>

          <Input
            placeholder="Search AHL teams..."
            value={searchQuery}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          <div className="flex flex-wrap justify-center gap-2 mb-4">
                            {team.awards.slice(0, 3).map((award: any) => (
                              ...
                              >
                                ...
                                {award.award_type === "AHL Cup" ? "Cup" : "Trophy"} {award.year}
                              </Badge>
                            ))}
            ...
            {filteredTeams.length === 0 && (
              <div className="col-span-full text-center py-12">
                <p className="text-muted-foreground">No AHL teams found matching your search.</p>
              </div>
            )}
          </div>
        )}
