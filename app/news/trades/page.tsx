  ...
  useEffect(() => {
    ...
    const fetchPlayers = async () => {
      try {
        ...
        const { data: seasonRegs, error: seasonRegsError } = await supabase
        ...
        if (mounted) {
          ...
          data?.forEach((player) => {
            const seasonReg = seasonRegs?.find((reg) => reg.user_id === player.user_id)
            ...
            playerMap[player.id] = enhancedPlayer
          })
